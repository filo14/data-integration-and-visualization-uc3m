    ...
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                ...
                print(
                    "Ensure your Docker container is running and the port 5432 is exposed."
                )
                raise e
...
def load_data(conn, t_country, t_population, t_crime, t_immig):
    def insert_data(conn, table_name, columns, data_to_insert, conflict_rule=""):
        ...
        try:
            insert_query = (
                f"INSERT INTO {table_name} {columns} VALUES %s {conflict_rule}"
            )
    ...
    def load_crime(conn, t_crime):
        ...
        insert_data(
            conn,
            "crime",
            "(convicts_per_100000, country_iso3_id, year_id)",
            data_to_insert,
            conflict_rule,
        )
    ...
    def load_immig(conn, t_immig):
        ...
        insert_data(
            conn,
            "immigration",
            "(immigration_per_100000, country_iso3_id, year_id)",
            data_to_insert,
            conflict_rule,
        )
...
if __name__ == "__main__":
    ...
    if db_conn:
        try:
            ...
            # 2. T
            t_country, t_population, t_crime, t_immig = transform_data(
                raw_population_tuple, raw_crime, raw_immig
            )
